...
const deleteStory = async (req, res) => {
  try {
    ...
    if (!story) return res.status(404).json({ message: 'Story not found' });
    if (story.userId.toString() !== req.user.id) return res.status(403).json({ message: 'Unauthorized' });
...
const viewStory = async (req, res) => {
  try {
    ...
    const existingView = await StoryView.findOne({ storyId: req.params.id, userId: req.user.id });
    if (!existingView) {
      const view = new StoryView({ storyId: req.params.id, userId: req.user.id });
      await view.save();
...
const toggleLikeStory = async (req, res) => {
  try {
    const existingLike = await Like.findOne({ userId: req.user.id, contentId: req.params.id, contentType: 'story' });
    if (existingLike) {
      await existingLike.remove();
      res.json({ message: 'Unliked' });
    } else {
      const like = new Like({ userId: req.user.id, contentId: req.params.id, contentType: 'story' });
      await like.save();
